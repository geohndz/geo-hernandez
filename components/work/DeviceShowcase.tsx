...
const playButtonClass =
  "press-scale pointer-events-auto z-10 flex cursor-pointer items-center justify-center bg-[var(--line)] text-white backdrop-blur-[6px] hover:bg-[var(--line-strong)]";
...
export function DeviceShowcase({
  ...
  const playToggle = (
    <button
      ...
      className={cn(playButtonClass, "absolute bottom-4 right-4 h-10 w-10 rounded-full")}
    >
  ...
  const deviceToggle = !isVisor ? (
    <button
      ...
      className={cn(
        playButtonClass,
        "absolute bottom-3 left-[calc(100%+12px)] hidden h-10 w-10 rounded-full md:flex",
      )}
    >
      <PlayPauseGlyph playing={playing} />
    </button>
